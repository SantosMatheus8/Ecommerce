...
import { CreateUser, QueryUsers, UserAccessProfileResponse, UserResponse, UsersOrderByEnum } from "../../domain/dtos/user";
...
export class UserController {
  ...
  public async create(
    ...
    const simplifiedUser = this.toSimpleUserWithAccessProfile(user);
  ...
  private toSimpleUserWithAccessProfile(user: User): UserAccessProfileResponse {
    return {
      id: user.id,
      name: user.name,
      email: user.email,
      ...
      phoneNumber: user.phoneNumber,
      status: user.status,
      accessProfilesIds: user.accessProfiles.map((accessProfile) => accessProfile.id),
    };
  }
}
